  ...
  stages {
    stage('focal') {
      parallel {
        ...
        stage('focal-hybris-android9-arm64') {
          agent { label 'debos-amd64' }
          steps {
            // debos podman NEEDS /etc/apt-cacher-ng/99-ubports.conf!
            sh './debos-podman focal/ubuntu-touch-hybris-android9-rootfs.yaml --fakemachine-backend=kvm -m 2G --scratchsize 10G --cpus $(nproc --all) -e "http_proxy:http://$(ip route get 8.8.8.8 | head -1 | cut -d\' \' -f7):3142"'
